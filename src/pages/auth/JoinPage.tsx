...
import {
  ...
  PreferencesStep,
  PrivacyStep,
} from '@/components/forms/join/JoinSteps'
...
const stepUi = [PersonalStep, DemographicsStep, LifestyleStep, PreferencesStep, PrivacyStep]
...
export function JoinPage() {
  ...
  function toggleInterest(value: string) {
    setForm((currentForm) => ({
      ...currentForm,
      shoppingInterests: currentForm.shoppingInterests.includes(value)
        ? currentForm.shoppingInterests.filter((item) => item !== value)
        : [...currentForm.shoppingInterests, value],
    }))
  }

  function goNext() {
    const nextErrors = validateRegisterStep(form, step)
  ...
  async function submitForm() {
    const nextErrors = isLast ? validateRegisterForm(form) : validateRegisterStep(form, step)
    setErrors(nextErrors)
    if (Object.keys(nextErrors).length) {
      if (isLast) setStep(firstInvalidStep(form))
      return
    }
    setSubmitting(true)
    setFormError('')
  ...
  if (success) return <RegistrationSuccess onContinue={() => navigate('/panelist/dashboard')} />
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {isLast ? (
                  ...
                  <StepView form={form} errors={errors} update={update} toggleInterest={toggleInterest} />
                )}
              </motion.div>
            </AnimatePresence>
